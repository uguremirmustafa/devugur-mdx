...
> I would like to mention an important point about my Ubuntu setup. I prefer using tiling window managers over stacking window managers. The following steps won't be different for Gnome Desktop Environment (Ubuntu's default desktop environment) but yours will look a bit different.
...
I use _Source Code Pro_ as my terminal and VSCode font. You can get the same font files from my [.dotfiles](https://github.com/uguremirmustafa/.dotfiles) or from [Google Fonts.](https://fonts.google.com/specimen/Source+Code+Pro) After downloading the files, don't forget to put them in `.fonts` folder in the home.
